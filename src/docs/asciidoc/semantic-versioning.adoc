...
If you're behind a proxy, you can make gradle work by creating a gradle.properties file in the project root with the following sample content
[source]
.gradle.properties
----
...
<3> Add a nexus repository hosted on myserver, named myrepo{versiontype}, with versiontype being either empty or "-SNAPSHOT" (more on that on the nebula section)
